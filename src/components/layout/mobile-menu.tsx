...
export function MobileMenu({ links }: { links: { href: string; label: string }[] }) {
  ...
  return (
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger className="grid size-10 place-items-center rounded-full hover:bg-surface lg:hidden" aria-label="Abrir menu">
        <ListIcon size={24} />
      </Dialog.Trigger>
      <AnimatePresence>
        {open && (
            ...
            <Dialog.Content asChild forceMount>
              ...
              >
                <div className="flex justify-end">
                  <Dialog.Close className="grid size-10 place-items-center rounded-full hover:bg-surface" aria-label="Fechar menu">
                    <XIcon size={24} />
                  </Dialog.Close>
                </div>
                ...
                <div className="mt-auto space-y-4">
                  <p className="text-muted">
                    Seja membro Dream: acesso antecipado aos drops, 10% na primeira compra e frete grátis acima de R$ 299.
                  </p>
